...
from vistas.reservas_ui import Ui_Dialog
from modelos.datos import ReservasDAO, SalonesDAO, TiposCocinaDAO, TiposReservasDAO, ClientesDAO
from controladores.reserva_create_edit_controller import ReversaController
from utilidades.message_box import MessageBox

class ReservasController(QDialog):
    def __init__(self):
        """
    ...
    def init_daos(self):
        ...
        try:
            ...
            self.dao_clientes = ClientesDAO()  # DAO para los clientes
    ...
    def config_events(self):
        ...
        self.ui.vcbtnVerDetalle.clicked.connect(lambda: self.open_modal(False))  # Abre el modal de modificación de reserva
    ...
    def config_table(self):
        ...
        self.dao_reserva = ReservasDAO()  # DAO para las reservas
        ...
        reservas = self.dao_reserva.get_by_salon_id(self.salon_selecionado)  # Obtiene las reservas para el salón seleccionado
        tipos_reserva = self.dao_tipo_reserva.get_all()  # Obtiene todos los tipos de reserva

        clientes = self.dao_clientes.get_all()  # Obtiene todos los tipos de reserva
        ...
        # Crear un diccionario para mapear cliente_id -> (nombre, teléfono)
        mapa_clientes = {cliente.id: (cliente.nombre, cliente.telefono) for cliente in clientes}
        ...
        for reserva in reservas:
            ...
            reserva.cliente_nombre, reserva.cliente_tlf = mapa_clientes.get(reserva.id_cliente, ("Desconocido", "No disponible"))
        ...
        for row, reserva in enumerate(reservas):
            ...
            self.model.setItem(row, 1, QStandardItem(reserva.cliente_nombre))  # Coloca el nombre de la persona en la segunda columna
            self.model.setItem(row, 2, QStandardItem(reserva.cliente_tlf))  # Coloca el teléfono en la tercera columna
        ...
        self.ui.vcGridReservas.verticalHeader().setMinimumSectionSize(35)  # Asegura que la última columna se estire
    ...
    def open_modal(self, nueva):
        ...
        if self.reserva_seleccionada != 0 or nueva:
            ...
            MessageBox("Seleccione una reserva para ver los datos", "warning").show()  # Muestra un mensaje de advertencia si no hay ninguna reserva seleccionada
